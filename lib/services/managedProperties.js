...
// Rassemble toutes les URLs d'image d'un bien (couverture + galerie + champs
// legacy) pour pouvoir les nettoyer sur Cloudinary à la suppression.
const collectImageUrls = (data) => {
  ...
  const legacy = [
    ...
    ...(Array.isArray(data.houseInsides) ? data.houseInsides : []),
  ]
  return Array.from(new Set([...fromImages, ...legacy].filter(Boolean)))
}
...
export const resolvePropertyImage = (data) => {
  const imgs = data.images
  let coverImage = ''
  if (Array.isArray(imgs)) {
    coverImage = imgs[0] || ''
  } else {
    coverImage = imgs?.cover || imgs?.gallery?.[0] || ''
  }
  return (
    coverImage ||
    data.imageUrl ||
    (Array.isArray(data.houseInsides) ? data.houseInsides[0] : '') ||
...
export const getManagedProperties = async () => {
  ...
  return snapshot.docs.map((d) => {
    const data = d.data()
    return { id: d.id, ...data, imageUrl: resolvePropertyImage(data) }
  })
}
...
export const getPropertiesByOwner = async (ownerId) => {
  ...
  results.flat().forEach((d) => {
    ...
    const mainImg = resolvePropertyImage(data)
...
export const deleteManagedProperty = async (id, collectionName = 'managed_properties') => {
  ...
  if (snap?.exists()) {
    const urls = collectImageUrls(snap.data())
