...
export default function AgentSubscriptionTab() {
  ...
  useEffect(() => {
    const load = async () => {
      setIsLoading(true)
      try {
        const current = await getAgentSubscriptionAmount()
  ...
  const onSubmit = async (e) => {
    ...
    setSaving(true)
    try {
      await setAgentSubscriptionAmount(amount)
  ...
  return (
    <form onSubmit={onSubmit} className="rounded-xl bg-white p-6 shadow-sm">
      <div className="mb-6">
        <h2 className="text-lg font-bold text-gray-900">Abonnement agents</h2>
        <p className="mt-1 text-sm text-gray-500">
          Montant mensuel facturé à chaque agent une fois sa candidature approuvée et sa période
          d'essai terminée
        </p>
      </div>
      ...
      >
        ...
        <p>
          Chaque agent bénéficie de {TRIAL_MONTHS} mois d'essai gratuit à compter de la
          validation de sa candidature, puis doit régler ce montant chaque mois pour rester
          référencé. Les paiements se saisissent manuellement depuis la fiche de l'agent.
        </p>
      </div>
